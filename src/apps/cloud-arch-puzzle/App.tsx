...
const CHALLENGES: Challenge[] = [
  ...
  },
  {
    ...
    explanation:
      'ブラウザはDNSでドメインを解決してからロードバランサーへ到達します。LBがトラフィックを分散し、Web→APIの経路で画面表示とビジネスロジックを処理。キャッシュで頻繁なクエリを高速化し、RDBでデータを永続化します。',
  },
  {
    id: 3,
    title: '動画配信プラットフォーム',
  ...
  },
  {
    ...
    explanation:
      'CDNを「静的アセット配信とAPI入口のルーティングをまとめるエッジ」（例: CloudFrontのオリジン振り分け）とみなした一本化です。DNS→CDN→WAF→API Gatewayの順で保護しつつバックエンドへ。認証後、APIがRDBで在庫管理し外部決済APIと連携。注文確定後はキューで非同期にワーカーが確認メールを送信します。',
  },
  {
    id: 6,
    title: 'SNSアプリケーション',
  ...
  },
  {
    ...
    explanation:
      '利用者はDNSでエンドポイントを解決してからWAFに到達します。CI/CDがコードをビルドしレジストリに保存、K8sにデプロイ。WAF→API Gatewayで安全にルーティングし、コンテナ内のAPIサービスが処理。監視とログ管理で本番運用の可観測性を確保します。',
  },
];
