...
const SERVICES = [
  { name: 'lb', label: 'ロードバランサー（入口）' },
  { name: 'api-gw', label: 'APIゲートウェイ（アプリ）' },
  { name: 'vpc', label: '仮想ネットワーク／VPC（ネットワーク）' },
  { name: 'compute', label: '仮想マシン・インスタンス（計算）' },
  { name: 'object-store', label: 'オブジェクトストレージ（データ）' },
] as const;
...
export default function CloudArchPuzzleApp() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-gray-800">クラウドアーキテクチャパズル</h2>
            ...
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {LAYERS.map(layer => (
                <div key={layer} className="text-sm">
                  ...
                  <span>{getServiceLabel(SOLUTION[layer])}</span>
                </div>
              ))}
            </div>
          </div>
